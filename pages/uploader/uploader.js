...
Page({
  ...
  upload: function() {
    if (!this.data.ifJump) {
      ...
      if (this.data.text.length > 100) {
        ...
        if (this.data.jiage != "") {
          this.data.temp.jiage = this.data.jiage
        } else {
          this.data.temp.jiage = "0"
        }
        if (this.data.xiaoliang.length > 0) {
          this.data.temp.xiaoliang = this.data.xiaoliang
        } else {
          this.data.temp.xiaoliang = "0"
        }
        ...
        if (this.data.ifEdit) {
          this.data.temp.id = app.msgList[this.data.editId].id
          this.data.temp._id = app.msgList[this.data.editId]._id
          app.msgList[this.data.editId] = this.data.temp
          this.cloudedit()
        } else {
          app.msgList[i] = this.data.temp
          this.cloudupload()
          app.sellList[i] = app.globalData.userInfo
        }
        this.openToast()
      }
    }
  ...
  cloudupload: function() {
    wx.cloud.uploadFile({
      cloudPath: 'sell/' + this.data.temp.id + '.png', // 上传至云端的路径
  ...
  cloudedit: function() {
    wx.cloud.uploadFile({
      cloudPath: 'sell/' + app.msgList[this.data.editId].id + '.png', // 上传至云端的路径
      filePath: this.data.temp.src, // 小程序临时文件路径
      success: res => {
        const db = wx.cloud.database()
        const sell = db.collection('sell')
        db.collection('sell').doc(app.msgList[this.data.editId]._id).update({
          data: {
            edit: false,
            id: this.data.temp.id,
            jiage: this.data.temp.jiage,
